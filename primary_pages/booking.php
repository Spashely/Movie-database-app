...
if (isset($_POST['Book'])) {
    ...
    $stock = $_POST['Stock'];
    $res = mysqli_query($connection, "SELECT * FROM Movies2 WHERE ID = '$id'");
    $row = mysqli_fetch_assoc($res);

    if ($row['Stock'] >= $stock) { /* If stock is enough */
        $newStock = $row['Stock'] - $stock;
        mysqli_query($connection, "UPDATE Movies2 SET Stock='$newStock' WHERE ID='$id'");
        session_start();
    ...
    $_SESSION['booking'] = [
          ...
          <div class='modal-dialog modal-dialog-centered'>
            <div class='modal-content'>
              <div class='modal-header'>
                <h5 class='modal-title'>Booking completed</h5>
                <button type='button' class='btn-close' data-bs-dismiss='modal'></button>
              </div>
              <div class='modal-body'>
                You successfully booked <strong>$stock</strong> ticket(s) for <strong>{$row['Title']}</strong>.
              </div>
              <div class='modal-footer'>
                <a href='booking_success.php' class='btn button3'>Print ticket</a>
              </div>
            </div>
          </div>
        </div>
        ...
        ";
        exit;
    } else { /* If not enough tickets */
        echo "Not enough tickets<br>";
    }
}

$result = mysqli_query($connection, "SELECT * FROM Movies2 WHERE ID = '$id'");
$row = mysqli_fetch_assoc($result);

echo "<form method='post' action='booking.php?ID=$id'>"; /* Dynamic content from DB */
echo "<div class='booking'>";
echo "<div class='booking-left'>";
echo "<img src='../images/{$row['Image']}'>";
echo "</div>";
echo "<div class='booking-right'>";
echo "<h1>{$row['Title']}</h1>";
echo "<p>{$row['Description']}</p>";
echo "<p>Age rating: {$row['Age_rating']}</p>";
echo "<div class='booking-actions'>";
echo "Tickets :<input type='text' name='Stock'><br/>";
echo "<br>";
if(isset($_SESSION['Age']) && $_SESSION['Age'] < 18){
    echo "<input type='submit' value='Book' class='btn button4' disabled>";
} 
else { 
  echo "<input type='submit' value='Book' name='Book' class='btn button4'>";
 } 
echo "</div>"; 
